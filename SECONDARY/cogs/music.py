    ...
    class Play:
            ...
            async def join_(self, interaction: discord.Interaction):
                ...
                if interaction.user.voice is None:
                    return await interaction.response.send_message(f'Não deu pra entrar no canal de voz não, **{random.choice(FRASE_MEIO)}**. Tenta entrar lá primeiro e me chama.')
                    ...
                    # If bot is already in a voice channel within that server
                        ...
                        if interaction.user.voice.channel.id == self.vc.channel.id:
                            return 0
                        return await interaction.response.send_message(f'A banda já está tocando em outro canal de voz, **{random.choice(FRASE_MEIO)}**.')
            ...
            async def play_(self, interaction: discord.Interaction, song: str):
                ...
                try:
                    ...
                    if url == None: return
                    pass
                self.vc.play(discord.PCMVolumeTransformer(discord.FFmpegPCMAudio(url, **self.FFMPEG_OPTIONS)), after=lambda error: self.bot.loop.create_task(self.check_queue(interaction)))
            ...
            async def now_(self, interaction: discord.Interaction):
                if not self.isPlaying: return await interaction.response.send_message(f'Não estamos com serviço couvert hoje, **{random.choice(FRASE_MEIO)}**. Obrigado.')
            ...
            async def musicInfo_(self, url: str):
